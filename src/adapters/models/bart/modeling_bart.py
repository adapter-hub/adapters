...
class BartAttentionWithAdapters(BartAttentionAdaptersMixin, BartAttention):
    ...
    def forward(
        ...
        # Note: With LoRA parallel composition, batch size may change after projection
        # Use actual tensor shape instead of predetermined bsz
        ...
        if is_cross_attention and past_key_values is not None and is_updated:
            ...
            # Use actual tensor batch size for LoRA parallel composition support
            key_states = key_states.view(key_states.shape[0], src_len, self.num_heads, self.head_dim).transpose(1, 2)
            value_states = value_states.view(value_states.shape[0], src_len, self.num_heads, self.head_dim).transpose(
                1, 2
            )
        ...
        # Get actual batch size from attn_output (may differ due to parallel composition)
        # but use original tgt_len (prefix tuning doesn't change query sequence length)
