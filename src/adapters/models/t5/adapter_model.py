...
@inherit_doc_for_adapter_model(
    model=T5Model,
    custom_intro="""T5 Model with the option to add multiple flexible prediction heads on top.""",
)
class T5AdapterModel(
    ...
    def load_state_dict(self, state_dict, strict=True):
        """
        Custom state_dict loading to handle compatibility between T5ForConditionalGeneration
        (which has encoder/decoder directly) and T5AdapterModel (which wraps them in transformer).
        """
        ...
        if has_encoder_prefix and not has_transformer_prefix:
            # Keys are from a model like T5ForConditionalGeneration - need to add transformer. prefix
            new_state_dict = {}
    ...
    @inherit_doc_for_function(T5Model.forward)  # Apply new decorator here
    @ForwardContext.wrap
    def forward(
        self,
        input_ids=None,
