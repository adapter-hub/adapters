...
from transformers.models.whisper.modeling_whisper import WhisperAttention, WhisperEncoderLayer, WhisperDecoderLayer

from .mixin_whisper import WhisperAttentionAdaptersMixin, WhisperEncoderLayerAdaptersMixin, \
    WhisperDecoderLayerAdaptersMixin
from ...composition import adjust_tensors_for_parallel, match_attn_matrices_for_parallel, adjust_tensors_for_parallel_
...
class WhisperAttentionWithAdapters(WhisperAttention, WhisperAttentionAdaptersMixin):
    ...
    def forward(
        ...
        # bsz = batch size
        # tgt_len = target length (length of sequences in batch)
        ...
        if (
            ...
            # cross_attentions
            # Get new key and value projections and transform them to have the same shape as the query
            key_states = self._shape(self.k_proj(key_value_states), -1, bsz)
            value_states = self._shape(self.v_proj(key_value_states), -1, bsz)
        elif past_key_value is not None:
            # reuse k, v, self_attention
            # Get new key and value projections and concat them with the previous key and value projections
            ...
            # self_attention
            # No cross_attention, only self attention -> use `hidden_states` for `key_states` and `value_states`
            key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
            value_states = self._shape(self.v_proj(hidden_states), -1, bsz)
        ...
        # Compute attention weights with key and query states
