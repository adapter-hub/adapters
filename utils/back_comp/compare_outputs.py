...
for model_name in get_model_names():
    ...
    for adapter_config in get_new_adapter_config_strings():
        ...
        adapter_name = model.load_adapter(
            os.path.join(model_dir, "weights_" + adapter_config)
        )
        ...
        ref_output = restore_from_jsonl(
            config=adapter_config, file_path=os.path.join(model_dir, "output.jsonl")
        )
        is_equal = compare_lists_close(
            ref_output, model_output_n, rtol=1e-05, atol=1e-08
        )
