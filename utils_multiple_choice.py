...
class CosmosProcessor(DataProcessor):
    ...
    def get_test_examples(self, data_dir):
        ...
        logger.info("LOOKING AT {} test".format(data_dir))
        raise ValueError(
            "For cosmosqa testing, the input file does not contain a label column. It can not be tested in current code"
            "setting!"
        )
        return self._create_examples(self._read_csv(os.path.join(data_dir, "test.csv")), "test")
    ...
    def _create_examples(self, lines: List[List[str]], type: str):
        ...
        examples = [
            InputExample(
                example_id=line[0],
                question=line[2],  # in the cosmosqa dataset, the
                # common beginning of each
                # choice is stored in "sent2".
...
class CommonsenseQAProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, type):
        ...
        for line in lines:
            ...
            context = tokenization.convert_to_unicode(line['question']['question_concept'])
            answers = np.array([
    ...
    for (ex_index, example) in tqdm.tqdm(enumerate(examples), desc="convert examples to features"):
        ...
        for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
            ...
            inputs = tokenizer.encode_plus(
                ...
                # padding="max_length" --> Not used in 2.11 !! 아아악!! --> collate size mismatch (batch)
                truncation=True,
